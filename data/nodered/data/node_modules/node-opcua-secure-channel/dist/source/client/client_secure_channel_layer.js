...
class ClientSecureChannelLayer extends events_1.EventEmitter {
    ...
    _on_message_received(response, msgType, requestId) {
        ...
        if (response.responseHeader.requestHandle !== requestId) {
            ...
            errorLog(chalk_1.default.red.bgWhite.bold("xxxxx  <<<<<< _on_message_received  ERROR"), "requestId=", requestId, (_a = this._requests[requestId]) === null || _a === void 0 ? void 0 : _a.constructor.name, "response.responseHeader.requestHandle=", response.responseHeader.requestHandle, response.schema.name.padStart(30));
        }
        /* istanbul ignore next */
    ...
    _performMessageTransaction(msgType, request, callback) {
        ...
        const modified_callback = (err, response) => {
            /* istanbul ignore next */
            if (doDebug) {
                debugLog(chalk_1.default.cyan("------------------------------------- Client receiving response "), request.constructor.name, request.requestHeader.requestHandle, response ? response.constructor.name : "null", "err=", err ? err.message : "null", "securityTokenId=", this.securityToken ? this.securityToken.tokenId : "x");
            }
    ...
    _internal_perform_transaction(transactionData) {
        ...
        if (request.requestHeader.requestHandle !== exports.requestHandleNotSetValue) {
            errorLog(chalk_1.default.bgRed.white("xxxxx   >>>>>> request has already been set with a requestHandle"), requestHandle, request.requestHeader.requestHandle, request.constructor.name);
    ...
    _sendSecureOpcUARequest(msgType, request, requestId) {
        ...
        if (request.requestHeader.requestHandle !== options.requestId) {
            doDebug &&
                debugLog(chalk_1.default.red.bold("------------------------------------- Invalid request id"), request.requestHeader.requestHandle, options.requestId);
        }
        request.requestHeader.returnDiagnostics = 0x0;
        /* istanbul ignore next */
        if (utils_1.doTraceClientRequestContent) {
            warningLog(chalk_1.default.yellow.bold("------------------------------------- Client Sending a request  "), request.constructor.name, "h=", request.requestHeader.requestHandle, " channel id ", this.channelId, " securityToken=", this.securityToken ? this.securityToken.tokenId : "x");
        }
        if (utils_1.doTraceClientRequestContent) {
            warningLog(request.toString());
        }
